# scoring for Tetouan city power consumption predictions
# usage: python3 score_submission.py --submission-path submission.csv --solution-path solution.csv
# outputs a single SMAPE float (lower = better, 0 is perfect)
...
def load_and_validate(sub_path, sol_path):
    ...
    # coerce to float
    try:
        sub["zone_1_power"] = sub["zone_1_power"].astype(float)
    except (ValueError, TypeError):
        sys.exit("ERROR: zone_1_power column contains non-numeric values")

    if sub["zone_1_power"].isna().any():
        sys.exit("ERROR: zone_1_power column contains NaN values")

    # sanity check: power should be positive and in a reasonable range
    if (sub["zone_1_power"] <= 0).any():
        sys.exit("ERROR: zone_1_power contains non-positive values")

    if (sub["zone_1_power"] > 200000).any():
        sys.exit("ERROR: zone_1_power contains implausibly large values (>200000 kW)")
...
def smape(y_true, y_pred):
    ...
    # avoid division by zero (shouldn't happen with power data)
    mask = denominator > 0
...
def score(sub_path, sol_path):
    ...
    y_true = sol["zone_1_power"].astype(float).values
    y_pred = sub["zone_1_power"].astype(float).values

    return round(smape(y_true, y_pred), 6)
